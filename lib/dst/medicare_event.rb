  ...
  class MedicareEvent < ::Sequel::Model(DST::DB)
    ...
    dataset_module do
      ...
      def current_lisl_date(date: Date.today)
        where(Sequel.lit('li_sub_dt <= ?', date))
      ...
      def current_lis_seq_num(date: Date.today)
        from_self(:alias => :m)
            .join(current_lisc_date(date: date), {
                :lisc_member_id => Sequel.qualify('m', 'memb_id'),
                :max_li_cpy_dt  => Sequel.qualify('m', 'li_cpy_dt')
            }, :table_alias => :lisc)
